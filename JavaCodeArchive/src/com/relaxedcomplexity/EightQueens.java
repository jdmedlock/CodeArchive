...
public class EightQueens {
     ...
     *   <li>The positions row must not already contain a Queen 
     *   <li>The positions column must not already contain a Queen
     ...
     *       coordinates:
     *         <ul>
     *         <li>(r+1,c+1) : Only rows 1 thru 7 and columns 1 thru 7
     *         <li>(r+1,c-1) : Only rows 1 thru 7 and columns 2 thru 8
     *         <li>(r-1,c+1) : Only rows 2 thru 8 and columns 1 thru 7
     *         <li>(r-1,c-1) : Only rows 2 thru 8 and columns 2 thru 8
     *         </ul>
     *   </ul>
	 * 
	 ...
	 */
	public static boolean testPosition(int rowPos, int colPos) {
		boolean testResult = false;
		
		if (chessBoard[rowPos][colPos] == OCCUPIED) {
			return false;
		}
		
		if (rowOccupied[rowPos]||colOccupied[colPos]) {
			return false;
		}
		
		if (rowPos < NROWS && colPos < NCOLS) {
			testResult = testPosition(rowPos+1,colPos+1);
		}
		if (rowPos < NROWS && colPos > 1) {
			testResult |= testPosition(rowPos+1,colPos-1);
		}
		if (rowPos > 1     && colPos < NCOLS) {
			testResult |= testPosition(rowPos-1,colPos+1);
		}
	 ...
	 */
	public static void printSolution(solutionNo) {
